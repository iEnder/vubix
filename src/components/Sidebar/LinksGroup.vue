...
<div class='links-group' >
    ...
    <ul class='links-group__list'>
        <li v-for="item in links" class='links-group__item' :id="'item-' + item.name.replaceAll('[^a-zA-Z\d\s:]', '').replaceAll(' ', '-').toLowerCase()">


            <div v-if="item.rest">
                <div class='links-group__container' @click="toggleSub(item.name.replaceAll('[^a-zA-Z\d\s:]', '').replaceAll(' ', '-').toLowerCase())">
                    <div  class='links-group__container--category'>
                        ...
                        <span>
                            ...
                            <span :id="'drop-'+item.name.replaceAll('[^a-zA-Z\d\s:]', '').replaceAll(' ', '-').toLowerCase()" class="links-group__item--dropdown"><i class="fa fa-chevron-down" aria-hidden="true"></i></span>
                        </span>
                        
                    </div>
                </div>
                <LinksGroup :id="'sub-'+item.name.replaceAll('[^a-zA-Z\d\s:]', '').replaceAll(' ', '-').toLowerCase()" class="subgroup" :links="item.rest" />
            </div>
